...
(defmethod print-subrip ((object cons) &optional (index 1) stream)
  (if (integerp index)
      ...
      (let ((stream (claraoke-internal:output-stream-from-designator stream))
        ...
        (terpri stream)
        (terpri stream)
        (format stream "~@[~&STYLE~%~{~A~%~}~%~]" (cue-style object))
        (print-subrip object 1 stream))))

(defun cue-style (object)
  (let ((all (loop for dialogue in object
                   for list-of-batch = (claraoke:overrides dialogue)
                   unless (null list-of-batch)
                     append (loop for batch in list-of-batch
                                  for item = (when (typep batch 'claraoke-text:batch)
                                               (claraoke:find-modifier batch :color1))
                                  unless (or (null item)
                                             (null (claraoke:arg1 item)))
                                    collect (claraoke:arg1 item))))
        (cue (lambda (string &aux (color (claraoke:color string)))
               (format nil "::cue(.~(~2,'0X~2,'0X~2,'0X~@[~2,'0X~]~)) ~
                           { color: #~@*~(~2,'0X~2,'0X~2,'0X~@[~2,'0X~]~); }"
                       (claraoke:red color)
                       (claraoke:green color)
                       (claraoke:blue color)
                       (claraoke:alpha color)))))
    (mapcar cue (remove-duplicates all :test 'string-equal))))
    ...
    (let ((arg (claraoke:arg1 object)))
      (cond ((null arg)
            ...
            (t (let* ((color (claraoke:color arg))
                      ...
                      (fmt (if (null *webvtt-language*)
                               "<font color=\"#~2,'0X~2,'0X~2,'0X~@[~2,'0X~]\">"
                               "<~(c.~2,'0X~2,'0X~2,'0X~@[~2,'0X~]~)>")))
                 (princ (coerce (pop *newline-stack*) 'string) stream)
                 (format stream fmt r g b a)
                 (push (if (null *webvtt-language*) "</font>" "</c>") *closer-stack*))))))
